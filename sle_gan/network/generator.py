...
class Generator(tf.keras.models.Model):
    """
    Input of the Generator is in shape: (B, 256 + num_classes)
    num_classes: 条件ベクトルの次元
    """
    ...
    def call(self, inputs, training=None, mask=None):
        """
        inputs = ノイズベクトル + 条件ベクトル。サイズがオリジナルSLE-GANの256ではなくなるので、ユニット数256のDenseを入れる。
        その出力を本来のinput_blockへ入力する。
        """
        # inputs : (B, 256 + num_classes)
        x = self.input_dense(inputs) # --> (B, 256)
        x = self.reshape(x) # --> (B, 1, 1, 256)
        
        x = self.input_block(x)  # --> (B, 4, 4, 1024)
